...
type
  ...
  public
    ...
    property WordList: TSkyStringList read FWordList;
  end;
...
begin
  ...
  begin
    ...
    if TheChar in ConstWordChars then
      Break;
    Inc(AnIndex);
    if TheChar = ' ' then
      Break;
...
begin
  ...
  begin
    TheChar := AString[AnIndex];
    if not(TheChar in ConstWordChars) then
      Break;
    TheWord := TheWord + TheChar;
    Inc(AnIndex);
  end;
...
begin
  ...
  begin
    ...
    if (WordList[I] = '''') then
      if (WordList[I - 1][1] in ConstWordChars) and(
          // 1'st
          (WordList[I + 1] = 'st') or
          // 2'nd
          (WordList[I + 1] = 'nd') or
      ...
      else if SameText(WordList[I - 1], 'that') and(SameText(WordList[I + 1], 's')) then
      begin
        WordList[I - 1] := WordList[I - 1] + WordList[I] + WordList[I + 1];
        WordList.DeleteFromIndex(I + 1);
        WordList.DeleteFromIndex(I);
...
begin
  ...
  begin
    ...
    begin
      ...
      if (not TheInQuote) and (TheChar in ['!', '?']) and (Length(TheSentence) > 1) then
        Break;
    end;
