...
begin
  ...
  begin
    ...
    TheIndex := GetInstance.FFieldNamesList.AddObject(TheNewValue, TObject(GetInstance.FCurrentId));
  end;
...
var
  ...
  TheCount: Integer;
begin
  SetLength(SomeEntityClasseNames, Length(SomeTokens));
  SetLength(SomeFieldNames, Length(SomeTokens));
  TheCount := 0;
  for I := 0 to High(SomeTokens) do
  begin
    if GetInstance.FFieldNamesList.Find(SomeTokens[I].TokenString, TheIndex) then
    begin
      SomeEntityClasseNames[TheCount] := GetInstance.FFieldNamesList.Items[TheIndex];
      SomeFieldNames[TheCount] := GetFieldNameForEntityTokenString(GetInstance.FFieldNamesList.Items[TheIndex]);
      Inc(TheCount);
    end
    else
      raise ESkyEntityFieldNameTokenNotFound.Create(GetInstance, 'GetTokensInfo', SomeTokens[I].TokenString);
  end;
  SetLength(SomeEntityClasseNames, TheCount);
  SetLength(SomeFieldNames, TheCount);
end;
