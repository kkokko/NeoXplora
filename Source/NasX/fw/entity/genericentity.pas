...
type
  ...
  public
    ...
    function GetFieldInfos(IgnoreId: Boolean = False): {$IFDEF VER210}TFieldInfo.TArray{$ELSE}TFieldInfoArray{$ENDIF}; override;
    ...
    procedure SetValuesForFields(const SomeKeyValues: {$IFDEF VER210}TKeyValue.TArray{$ELSE}TKeyValueArray{$ENDIF});
...
function TGenericEntity.GetFieldInfos(IgnoreId: Boolean): {$IFDEF VER210}TFieldInfo.TArray{$ELSE}TFieldInfoArray{$ENDIF};
var
  TheCount: Integer;
  I: Integer;
begin
...
begin
  ...
  for I := 0 to FFieldValues.Count - 1 do
    ...
    begin
      Result[TheCount] :=  FFieldValues[I];
      Inc(TheCount);
    end;
  SetLength(Result, TheCount);
end;
...
procedure TGenericEntity.SetValuesForFields(const SomeKeyValues: {$IFDEF VER210}TKeyValue.TArray{$ELSE}TKeyValueArray{$ENDIF});
var
  I: Integer;
begin
