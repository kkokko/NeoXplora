...
const
  ...
  MaxVariantListSize = MaxInt div 40;
...
{$Region ' TSkyStringList'}
  ...
  protected
    ...
    function CompareStrings(const AString1, AString2: TSkyString): Integer;
  ...
  public
    ...
    procedure LoadFromFile(const FileName: TSkyString); virtual;
    procedure LoadFromStream(Stream: TStream); virtual;
    procedure SaveToFile(const FileName: TSkyString); virtual;
...
{$Region ' TSkyStringStringList'}
  TSkyStringStringList = class(TSkyStringList)
  private
  ...
  protected
    ...
    procedure SetCapacity(NewCapacity: Integer); override;
    procedure SetTextStr(const Value: TSkyString); override;
  ...
  public
    ...
    procedure AddMultiple(const SomeItems: array of TSkyString; const SomeLinks: array of TSkyString); reintroduce;
    function GetAllItems: TStringArray; reintroduce;
    function GetTextStr: TSkyString; override;
    procedure Clear; override;
    procedure DeleteFromIndex(Index: Integer); override;
    procedure Delete(const AString: TSkyString); reintroduce; virtual;
    function Find(const AString: TSkyString; out Index: Integer): Boolean; reintroduce; virtual;
...
uses
  TypesFunctions, ExceptionClasses, Variants, Math;
...
procedure TSkyList.SetCount(AValue: Integer);
begin
  while FCount > AValue do
    DeleteFromIndex(FCount - 1);
end;
...
begin
  AddMultiple(SomeItems, SomeLinks);
end;
...
begin
  ...
  try
    ...
    SetLength(Result, TheTempList.Count);
...
begin
  Result := inherited AddObject(AClassName, TObject(AClassType));
end;
...
begin
  Index := IndexOfObject(TObject(AClassType));
  Result := Index <> -1;
end;
...
begin
  Result := inherited IndexOfObject(TObject(AClassType));
end;

procedure TSkyClassTypeList.PutItem(AIndex: Integer; AClass: TClass);
begin
  inherited PutLink(AIndex, TObject(AClass));
end;
{$EndRegion}
{$Region 'TSkyIntegerList'}
