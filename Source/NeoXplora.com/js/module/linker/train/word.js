  ...
  methods: {
    ...
    setup: function () {
    	$(document).ready(function() {
    		$(NeoX.Modules.WordControl.getConfig().Controls.self).parent(NeoX.Modules.WordControl.getConfig().Controls.self).each(function() {
          $(this).css('padding', 0);
        });
    	});      
    },
    ...
    hookEvents: function() {
      NeoX.Modules.WordControl.hookEvent("click", NeoX.Modules.WordControl.getConfig().Controls.self, NeoX.Modules.WordControl.switchColors);
    },
    
    switchColors: function() {
      if($(NeoX.Modules.EntityControl.getConfig().Controls.active).length) {
        var word = $(this);
        var entity = $(NeoX.Modules.EntityControl.getConfig().Controls.active).find(NeoX.Modules.EntityControl.getConfig().Controls.portrait);
