...
class TPanelIRepRules extends TPanel {
  ...
  public function postRuleName() {
    if(isset($_REQUEST['ruleId']) && isset($_REQUEST['ruleName'])) {
      ...
      if($ruleId == -1) {
        ...
        $result = $this->core->entity("ireprule")->insert(array(
          "name",
          "order"
        ), array( array(
            $ruleName,
  ...
  public function deleteRule() {
    ...
    $query = $this->core->entity("ireprule")->deleteRuleWithData($ruleId);
  ...
  private function getRuleConditions($ruleId) {
    $result = $this->core->entity("ireprulegroup")->getRuleMainGroup($ruleId);
  ...
  private function getConditionGroupChildren($groupId) {
    ...
    $children = array();
    $result = $this->core->entity("ireprulegroup")->getGroupChildren($groupId);
    ...
    while($record = $result->fetch_array()) {
      $children[intval($record['Order'])] = array(
        'id' => $record['Id'],
        'KeyPropertyType' => $record[Entity\TIRepRuleCondition::$tok_keyPropertyType],
        'PropertyKey' => $record[Entity\TIRepRuleCondition::$tok_propertyKey],
        'PropertyValue' => $record[Entity\TIRepRuleCondition::$tok_propertyValue],
        'OperandType' => $record[Entity\TIRepRuleCondition::$tok_operandType]
      );
    }
    ksort($children);
    return $children;
  ...
  private function insertConditionsGroup($ruleId, $order, $parentId, $conjunctionType) {
    if($parentId != null) {
      $result = $this->core->entity("ireprulegroup")->insert(
        array(
          "ruleId",
          "order",
          "parentId",
  ...
  private function updateConditionsGroup($groupId, $order, $parentId, $conjunctionType) {
    if($parentId != null) {
      $result = $this->core->entity("ireprulegroup")->update($groupId, array(
        "order" => $order,
        "parentId" => $parentId,
  ...
  private function deleteConditionsGroup($groupId) {
    ...
    $result = $this->core->entity("ireprulegroup")->select(array('parentId' => array($groupId)), "*");
    while($subGroup = $result->fetch_array()) {
      $this->deleteConditionsGroup($subGroup['Id']);
    }
    $result = $this->core->entity("ireprulegroup")->delete($groupId);
    return $result;
  }
  ...
  private function updateRuleCondition($conditionId, $groupId, $order) {
    $result = $this->core->entity("ireprulecondition")->update($conditionId, array(
      "groupId" => $groupId,
      "order" => $order
    ));
    return $result;
  ...
  private function updateRuleValue($ruleId, $valueData) {
    ...
    if($actionType == "delete") {
      ...
      $result = $this->core->entity("ireprulevalue")->insert(array(
        "ruleId",
        "keyPropertyType",
        "propertyKey",
        "operandType",
  ...
  private function getRuleValues($ruleId) {
    ...
    foreach($valueList as $data) {
      $result["data"][] = array(
        "Id" => $data["Id"],
        "KeyPropertyType" => $data[Entity\TIRepRuleValue::$tok_keyPropertyType],
        "PropertyKey" => $data[Entity\TIRepRuleValue::$tok_propertyKey],
        "PropertyValue" => $data[Entity\TIRepRuleValue::$tok_propertyValue],
        "OperandType" => $data[Entity\TIRepRuleValue::$tok_operandType]
      );
    }
    return $result;
  }
