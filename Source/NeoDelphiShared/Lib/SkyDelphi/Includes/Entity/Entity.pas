...
uses
  TypesConsts, Classes, Variants, StringArray, Uni;
...
type
  ...
  private
    class var
      ...
      FEntityToken_Id: TEntityFieldNamesToken;
      FEntityToken_Name: TEntityFieldNamesToken;
    function GetId: TId;
    var
      FId: TId;
 ...
 {$ENDIF}
  ...
  public
    ...
    class function EntityToken_Id: TEntityFieldNamesToken; virtual;
    class function EntityToken_Name: TEntityFieldNamesToken; virtual;
    ...
    function GetFieldInfos(IgnoreId: Boolean = False): {$IFDEF UNICODE}TFieldInfo.TArray{$ELSE}TFieldInfoArray{$ENDIF};virtual;
    function EvaluateFieldInfos: {$IFDEF UNICODE}TFieldInfo.TArray{$ELSE}TFieldInfoArray{$ENDIF};
    ...
    function GetAsKeyValues: {$IFDEF UNICODE}TKeyValue.TArray{$ELSE}TKeyValueArray{$ENDIF}; virtual;
    class function RowIsRelevantForClass(ARow: TUniQuery): Boolean; virtual;
    class function GetRelevantClassForRow(ARow: TUniQuery): TEntityClass; virtual;
...
uses
  ...
  TypesFunctions, EntityList{$IFNDEF PACKAGE}, EntityJsonWriter{$ENDIF}, SkyIdList, SkyLists;
...
var
  I: Integer;
  TheFieldInfos: {$IFDEF UNICODE}TFieldInfo.TArray{$ELSE}TFieldInfoArray{$ENDIF};
...
function TEntity.EvaluateFieldInfos: {$IFDEF UNICODE}TFieldInfo.TArray{$ELSE}TFieldInfoArray{$ENDIF};
var
...
function TEntity.GetAsKeyValues: {$IFDEF UNICODE}TKeyValue.TArray{$ELSE}TKeyValueArray{$ENDIF};
var
  I: Integer;
  TheFieldNames: TStringArray;
begin
  TheFieldNames := GetFieldNames;
  SetLength(Result, TheFieldNames.Count);
...
class function TEntity.EntityToken_Id: TEntityFieldNamesToken;
begin
  Result := FEntityToken_Id;
end;

class function TEntity.EntityToken_Name: TEntityFieldNamesToken;
begin
  Result := FEntityToken_Name;
end;
    ...
    for I := 0 to High(TheFieldInfos) do
      if TheFieldInfos[I].FieldKind = tkClass then
        ...
        begin
          ...
          SetObjectProp(Self, TheFieldInfos[I].FieldName, TheObject);
        end else if TSkyStringList.ClassName = TheFieldInfos[I].FieldType then
        begin
          TheObject := TSkyStringList.Create(True);
          SetObjectProp(Self, TheFieldInfos[I].FieldName, TheObject);
        end
...
begin
  ...
  if TheClassType = nil then
    raise ESkyClassNotRegistered.Create('TEntityJsonStreamer', 'ReadEntityFromNode', AClassName);
  if (AnExpectedClassType <> nil) and not (TheClassType.InheritsFrom(AnExpectedClassType)) then
      raise ESkyInvalidClassType.Create(nil, 'TEntityJsonStreamer.ReadEntityFromNode',
...
var
  ...
  TheStringList, TheNewStringList: TSkyStringList;
  TheClassType: TEntityClass;
begin
  if (AFieldInfo.FieldKind = tkClass) then
    ...
    end else if (TSkyStringList.ClassName = AFieldInfo.FieldType) then
    begin
      GetObjectProp(Self, AFieldInfo.FieldName).Free;
      TheStringList := TSkyStringList.Create(True);
      TheNewStringList := TSkyStringList(TVarData(AEntity.GetValueForField(AFieldInfo.FieldName)).VPointer);
      TheStringList.CopyFrom(TheNewStringList);
      SetObjectProp(Self, AFieldInfo.FieldName, TheStringList);
    end
    else begin
      ...
      begin
        ...
        if TheClassType = nil then
          raise ESkyClassNotRegistered.Create('TEntityJsonStreamer', 'ReadEntityFromNode', AFieldInfo.FieldType);
...
begin
  ...
  begin
    ...
    for J := 0 to High(IgnoreFields) do
      if SameText(IgnoreFields[J].PropertyName, TheFieldInfos[I].FieldName) then
      begin
        TheFieldFound := True;
        Break;
      end;
...
function TEntity.GetFieldInfos(IgnoreId: Boolean = False): {$IFDEF UNICODE}TFieldInfo.TArray{$ELSE}TFieldInfoArray{$ENDIF};
begin
...
begin
  Result := GetFieldNames(IgnoreId).Count;
end;
...
begin
  SetLength(Result, Length(SomeEntities));
  for I := 0 to High(SomeEntities) do
    Result[I] := SomeEntities[I].GetValueForField(AToken.PropertyName);
end;
...
begin
  for I := 0 to High(SomeEntities) do
    Result.Add(SomeEntities[I].Name);
end;

class function TEntity.GetRelevantClassForRow(ARow: TUniQuery): TEntityClass;
begin
  Result := Self;
end;
...
class function TEntity.RowIsRelevantForClass(ARow: TUniQuery): Boolean;
begin
  Result := True;
end;
...
initialization
  ...
  TEntity.RegisterToken(TEntity.FEntityToken_Id, 'Id');
  TEntity.RegisterToken(TEntity.FEntityToken_Name, 'Name');
  TEntity(0).ToString;
