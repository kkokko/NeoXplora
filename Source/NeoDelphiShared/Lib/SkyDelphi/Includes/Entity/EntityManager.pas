...
uses
  SysUtils, ExceptionClasses, Windows;
...
begin
  if (not Assigned(AnEntityClass)) or TEntityClass(AnEntityClass).AutoManagedFields then
    raise ESkyFieldDoesNotExist.Create(nil, 'TEntityManager.GetFieldInfoClassForEntityClass', '');
...
begin
  if (not Assigned(AnEntityClass)) or TEntityClass(AnEntityClass).AutoManagedFields then
    raise ESkyFieldDoesNotExist.Create(nil, 'TEntityManager.GetFieldInfo', AFieldName);
...
begin
  if AnEntity.AutoManagedFields then
    raise ESkyFieldDoesNotExist.Create(nil, 'TEntityManager.GetEntityFieldInfos', '');
...
begin
  if (not Assigned(AnEntityClass)) or TEntityClass(AnEntityClass).AutoManagedFields then
    raise ESkyFieldDoesNotExist.Create(nil, 'TEntityManager.GetEntityFieldNames', '');
  TheFieldInfos := GetEntityFieldInfos(AnEntityClass, IgnoreId);
  for I := 0 to High(TheFieldInfos) do
    Result.Add(TheFieldInfos[I].FieldName);
end;
