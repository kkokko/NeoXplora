...
//       $where_condition=neighbour_algo_of_2($insentence);     //Algo type:Neighbour
 ...
 //Get DB into array
//
    $query = "select * from sentence WHERE ".$where_condition ;//. '  LIMIT 0 , 14';
//  $query = "select * from sentence  ". '  LIMIT 0 , 1';  
  // echo $query; die;
...
$start = microtime(true); // time count 
  ...
  $query2="insert into timelog (function,time) values ('Reps-update','".$sec."')";
  $result2 = $db->query($query2);
...
/**************************************/
  ...
  $time_taken = microtime(true) - $start; // end time counter
  $sec=$time_taken;///1000000;
  $query2="insert into timelog (function,time) values ('Predict','".$sec."')";
  $result2 = $db->query($query2);
