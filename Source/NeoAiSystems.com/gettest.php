...
function repguessfn($insentence,$percent) {
...
$j=0;
    for ($i = 0; $i < $num_results; $i++) {
        ...
        $score[$j] = SubFromDffs($sentenceCOPY[$j], $searchterm, $rep[$i], $diffHTML[$i], $subs[$i], $guess_rep[$i]);
if ( $score[$j] >= $percent){ // where $precentpage is 25%
$j++;
	}
    } //$i 
...
if ( $scorePOS[$j] >= $percent){ // where $precentpage is 25%
$j++;
	}		
        //SubFromDffs($sentenceCOPYPOS[$i], $searchterm, $rep[$i], $junk1, $junk2, $guess_repPOS[$i]);
...
$j=0;
    ...
    for ($i = 0; $i < $num_results; $i++) {
        $sentenceCOPYHYBRID[$j] = trim($sentence[$i]) . " ";        
        $scoreHYBRID[$j] = $score[$i] + $scorePOS[$i];
        $guess_repHYBRID[$j] = $guess_rep[$i];

if ( $scoreHYBRID[$j] >= $percent){ // where $precentpage is 25%
$j++;
	}			
...
if ($id==1)	{
	...
	$bestREPguess = repguessfn($row['sentence'],25);
