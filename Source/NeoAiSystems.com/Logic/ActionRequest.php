...
class TActionRequest {
  ...
  protected function update_status($sentenceID, $protoID = 0) {
    ...
    /*$values = array($fGen, $tSplit, $rSplit, $tRep, $rRep, $tCRep, $rCRep);
    
    $overZero = 0;
    $canProceed = true;

    for($i = 0; $i < count($values) - 3 && $canProceed; $i++) {
      if($values[$i] > 0) {
        for($j = $i + 3; $j < count($values); $j++) {
          if($values[$j] > 0) {
            $canProceed = false;
            break;
          }
    ...
    if(!$canProceed) {
      $this->query("UPDATE `page` SET `pageStatus` = 'psFinishedGenerate' WHERE `pageID` = '". $pageID ."'") or die(mysql_error());
      $this->query("UPDATE `sentence` SET `sentenceStatus` = 'ssFinishedGenerate' WHERE `pageID` = '". $pageID ."'") or die(mysql_error());
    } else {
    */
