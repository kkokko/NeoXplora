$(document).ready(function() {
  ...
  $(document).on('click', '.doneUnderstandButton', function() {
  	if($('.newRepValue').val() == '') {
  		if($('.rep-error').length) {
        $('.rep-error').html("<br/>Representation cannot be empty");
      } else {
        $('.understand-sentences').after("<div class='rep-error' style='color: red'><br/>Representation cannot be empty</div>");
      }
    } else {
      saveRep();
    }
  ...
  $(document).on('keypress', '.newRepValue', function(event) {
    if(event.which == 13) {
        ...
        if($('.rep-error').length) {
          ...
          $('.understand-sentences').after("<div class='rep-error' style='color: red'><br/><br/>Representation cannot be empty</div>");
        }
    	} else {
        saveRep();
    	}
...
function approveReviewRep(sentenceID, newValue) {
  $.ajax({
    ...
    success: function(json) {
        ...
        var near = newValue.substr(json['StrIndex'], newValue.length);
        $('.rep-error').remove();
        $('#s' + sentenceID + ' .newValue').after("<div class='rep-error' style='color: red'>" + json['ErrorString'] + " at \"" + near + "\"</div>");
      } else {
      	$('.rep-error').remove();
...
function approveAllReps(sentences) {
  $.ajax({
    ...
    success: function(json) {
    	if(json) {
    		for(var key in json) {
    			$('#s' + key + ' .rep-error').remove();
    			$('#s' + key + ' .newValue').after("<div class='rep-error' style='color: red'>" + json[key]['ErrorString'] + " at \"" + json[key]['StrIndex'] + "\"</div>");
    		}
      } else {
      	var page = parseInt($('.currentPage').html(), 10);
        if(!page) page = 1;
      ...
      $('.reviewrep td').each(function () {
      	var id = $(this).parent('tr').attr('id').replace('s', '');
        if(!json[id]) {
          $(this).animate({backgroundColor:'#A7E3A3'}, 300);
        } 
      });
    }
  });
