...
function GrepFromAlign($rep, $alignIN, $alignOUT)
{
	...
	for ($i=0;$i<count($alignIN);$i++)
	{
		 ...
		 {
		 $newrep=str_replace($alignOUT[$i],$alignIN[$i],$newrep);
		 }
...
function repguessfn($insentence, $subset) {
 ...
 //Get DB into array
//
 echo    $query = "select * from sentence WHERE ".$where_condition . '  LIMIT 0 , 14';
  // echo $query; die;
...
<?php 
    ...
    for ($i = 0; $i < $num_results; $i += $istep) {		
        ...
        $sentenceSW[$j] = trim($sentence[$i]) . " ";
      		...
      		if (strcmp(trim($searchterm),trim($sentenceSW[$j]))==0)   $scoreSW[$j] = 0;
		else   $scoreSW[$j] = posSWfn($searchterm,$sentenceSW[$j],$alignIN, $alignOUT);;

   	    
		$guess_repSW[$j] = GrepFromAlign($rep[$j],$alignIN, $alignOUT);
		
	//	if (strcmp ($alignIN, $alignOUT)==0) $scoreSW[$j]=0;

?>
<tr>
<td><?php echo $senIDSW[$j] ?></td>
<td><?php echo $sentenceSW[$j] ?></td>
<td><?php echo $scoreSW[$j] ?></td>
<td>

<?php 
for ($k=0;$k<count($alignIN);$k++)
	{
	echo " ". $alignIN[$k];
	} ?>
</td>
<td>
<?php for ($k=0;$k<count($alignOUT);$k++)
	{
	echo " ". $alignOUT[$k];
	} ?>

</td>
<td><?php echo $rep[$j] ?></td>
<td><?php echo $guess_repSW[$j] ?></td>
</tr>
<?php 	 
...
$story_data = "SELECT * from sentence where storyID='$cat'";
...
//echo $cat;
if ($percent=='25%'){
 $str="UPDATE page SET text25='$average1',pos25='$average2',hybrid25='$average3',persnt='$percent' WHERE storyID='$cat'";
}else if ($percent=='50%'){
$str="UPDATE page SET text50='$average1',pos50='$average2',hybrid50='$average3',persnt='$percent' WHERE storyID='$cat'";
}else if ($percent=='100%'){
$str="UPDATE page SET text100='$average1',pos100='$average2',hybrid100='$average3',persnt='$percent' WHERE storyID='$cat'";
}
