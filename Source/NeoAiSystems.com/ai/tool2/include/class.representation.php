...
class Representation extends POG_Base
{
	...
	*/
	function Get($representationId)
	{
		...
		{
			$this->representationId = $row['sentenceID'];
	...
	*/
	function GetList($fcv_array = array(), $sortBy='', $ascending=true, $limit='')
	{
		...
		$this->pog_query = "select * from `representation` ";
		...
		while ($row = Database::Read($cursor))
		{
			...
			$representation->representationId = $row['representationid'];
	...
	*/
	function Save($chageOn,$text)
	{
		...
		else
		{
			...
			'".$this->storylineId."' )";*/
		 $this->pog_query = "insert into `sentenceTBL` (`context_rep`, `storyID` ) values (
			'".$this->Escape($this->text)."', 
			'".$this->storylineId."' )"; 
		}
