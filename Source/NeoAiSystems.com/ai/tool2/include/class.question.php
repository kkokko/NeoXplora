...
class Question extends POG_Base
{
	...
	*/
	function Get($questionId)
	{
		...
		$this->pog_query = "select * from `qaTBL` where `questionid`='".intval($questionId)."' LIMIT 1";
		$cursor = Database::Reader($this->pog_query, $connection);
		while ($row = Database::Read($cursor))
		{
			$this->questionId = $row['questionid'];
			$this->statement = $this->Unescape($row['question']);
			$this->story_id = $this->Unescape($row['storyid']);
	...
	*/
	function GetList($fcv_array = array(), $sortBy='', $ascending=true, $limit='')
	{
		...
		$this->pog_query = "select * from `qaTBL` ";
		$questionList = Array();
		...
		while ($row = Database::Read($cursor))
		{
			...
			$question->statement = $this->Unescape($row['question']);
			$question->answer = $this->Unescape($row['answer']);
			$question->qarule = $this->Unescape($row['qarule']);
			$question->story_id = $this->Unescape($row['storyid']);
	...
	*/
	...
	{
		$connection = Database::Connect();
		$this->pog_query = "select `questionid` from `qaTBL` where `questionid`='".$this->questionId."' LIMIT 1";
		$rows = Database::Query($this->pog_query, $connection);
		if ($rows > 0)
		{
		$this->pog_query = "update `qaTBL` set 
			`question`='".$this->Escape($this->statement)."', 
			`storyid`='".$this->storyId."' where `questionid`='".$this->questionId."'";
			
		}
		else
		{
		$this->pog_query = "insert into `qaTBL` (`statement`, `storyid` ) values (
	...
	*/
	...
	{
		$connection = Database::Connect();
		$this->pog_query = "delete from `qaTBL` where `questionid`='".$this->questionId."'";
	...
	*/
	function DeleteList($fcv_array)
	{
		...
		{
			$connection = Database::Connect();
			$pog_query = "delete from `qaTBL` where ";
			for ($i=0, $c=sizeof($fcv_array); $i<$c; $i++)
			{
				if (sizeof($fcv_array[$i]) == 1)
				{
