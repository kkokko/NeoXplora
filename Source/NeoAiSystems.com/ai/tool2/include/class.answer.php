...
class Answer extends POG_Base
{
	...
	*/
	function Get($answerId)
	{
		...
		$this->pog_query = "select * from `qaTBL` where `questionid`='".intval($answerId)."' LIMIT 1";
		$cursor = Database::Reader($this->pog_query, $connection);
		while ($row = Database::Read($cursor))
		{
			$this->answerId = $row['questionid'];
			$this->statement = $this->Unescape($row['answer']);
	...
	*/
	...
	{
		$connection = Database::Connect();
		 $this->pog_query = "select `questionid` from `qaTBL` where `questionid`='".$this->answerId."' LIMIT 1";
		$rows = Database::Query($this->pog_query, $connection);
		if ($rows > 0)
		{
			$this->pog_query = "update `qaTBL` set 
			`answer`='".$this->Escape($this->statement)."' where `questionid`='".$this->answerId."'";
		}
		else
		{
			$this->pog_query = "insert into `qaTBL` (`answer`) values (
