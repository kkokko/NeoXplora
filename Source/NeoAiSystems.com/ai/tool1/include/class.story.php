...
class Story extends POG_Base
{
	...
	*/
	function Get($storyId)
	{
		...
		$this->pog_query = "select * from `story` where `storyid`='".intval($storyId)."' LIMIT 1";
	...
	*/
	function GetList($fcv_array = array(), $sortBy='', $ascending=true, $limit='')
	{
		...
		$this->pog_query = "select * from `story` ";
		$storyList = Array();
	...
	*/
	...
	{
		$connection = Database::Connect();
		$this->pog_query = "select `storyid` from `story` where `storyid`='".$this->storyId."' LIMIT 1";
		$rows = Database::Query($this->pog_query, $connection);
		if ($rows > 0)
		{
			$this->pog_query = "update `story` set 
		...
		{
			$this->pog_query = "insert into `story` (`title`, `body`, `categoryid`) values (
	...
	*/
	function Delete($deep = false, $across = false)
	{
		...
		$connection = Database::Connect();
		$this->pog_query = "delete from `story` where `storyid`='".$this->storyId."'";
	...
	*/
	function DeleteList($fcv_array, $deep = false, $across = false)
	{
		if (sizeof($fcv_array) > 0)
		{
			...
			{
				$connection = Database::Connect();
				$pog_query = "delete from `story` where ";
				for ($i=0, $c=sizeof($fcv_array); $i<$c; $i++)
				{
					if (sizeof($fcv_array[$i]) == 1)
					{
