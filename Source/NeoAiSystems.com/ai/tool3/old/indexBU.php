...
<?php
	...
	<!-- SEARCH -->	
			...
			If ($last == '/') {$last = "";}
			$html = "Target Sequence";           
                	$html .= "<input type=\"text\" name=\"searchEB\" value = $searchterm />
				  <input style=\"width: 75px;\" type=\"submit\" />";

			echo $html;


			$query = "select * from sentenceTBL where sentence like '%".$searchterm."%'";
			$result = $db->query($query);

			$num_results = $result->num_rows;

  			echo "<p>Number of sentence matches: ".$num_results."</p>";
